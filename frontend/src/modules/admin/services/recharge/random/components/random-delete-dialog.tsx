...
import { useDeleteRandomMutation } from "../hooks";
import { AlertTriangle, Trash2 } from "lucide-react";

export function RandomDeleteDialog() {
  const { isDeleteOpen, deletingId, deletingName, closeDelete } = useRandomModalStore();
  const deleteMutation = useDeleteRandomMutation();
    ...
    <Dialog open={isDeleteOpen} onOpenChange={(open) => !open && closeDelete()}>
      <DialogContent className="max-w-md">
        <DialogHeader className="flex flex-col items-center gap-2">
          ...
          <DialogTitle className="text-lg font-bold">Delete Random API</DialogTitle>
