...
import { useDeleteCategoriesMutation } from "../hooks";
import { AlertTriangle, Trash2 } from "lucide-react";

export function CategoriesDeleteDialog() {
  const { isDeleteOpen, deletingId, deletingName, closeDelete } = useCategoriesModalStore();
  const deleteMutation = useDeleteCategoriesMutation();
    ...
    <Dialog open={isDeleteOpen} onOpenChange={(open) => !open && closeDelete()}>
      <DialogContent className="max-w-md">
        <DialogHeader className="space-y-2">
          ...
          <DialogTitle className="text-lg font-bold">Delete Service Categories</DialogTitle>
          <DialogDescription>
