...
import { useDeleteRechargeMutation } from "../hooks";
import { AlertTriangle, Trash2 } from "lucide-react";

export function RechargeDeleteDialog() {
  const { isDeleteOpen, deletingId, deletingName, closeDelete } = useRechargeModalStore();
  const deleteMutation = useDeleteRechargeMutation();
    ...
    <Dialog open={isDeleteOpen} onOpenChange={(open) => !open && closeDelete()}>
      <DialogContent className="max-w-md">
        <DialogHeader className="flex flex-col items-center gap-2">
          ...
          <DialogTitle className="text-lg font-bold">Delete Recharge Commission</DialogTitle>
