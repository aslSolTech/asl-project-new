...
import { useDeleteFeedbackMutation } from "../hooks";
import { AlertTriangle, Trash2 } from "lucide-react";

export function FeedbackDeleteDialog() {
  const { isDeleteOpen, deletingId, deletingName, closeDelete } = useFeedbackModalStore();
    ...
    <Dialog open={isDeleteOpen} onOpenChange={(open) => !open && closeDelete()}>
      <DialogContent className="max-w-md">
        <DialogHeader className="space-y-2">
          ...
          <DialogTitle className="text-lg font-bold">Delete Feedback List</DialogTitle>
          <DialogDescription>
