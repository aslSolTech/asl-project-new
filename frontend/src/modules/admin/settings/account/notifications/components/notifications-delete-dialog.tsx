...
import { useDeleteNotificationsMutation } from "../hooks";
import { AlertTriangle, Trash2 } from "lucide-react";

export function NotificationsDeleteDialog() {
  const { isDeleteOpen, deletingId, deletingName, closeDelete } = useNotificationsModalStore();
    ...
    <Dialog open={isDeleteOpen} onOpenChange={(open) => !open && closeDelete()}>
      <DialogContent className="max-w-md">
        <DialogHeader className="space-y-2">
          ...
          <DialogTitle className="text-lg font-bold">Delete Notifications Config</DialogTitle>
          <DialogDescription>
