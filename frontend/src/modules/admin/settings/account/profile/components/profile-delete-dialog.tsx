...
import { useDeleteProfileMutation } from "../hooks";
import { AlertTriangle, Trash2 } from "lucide-react";

export function ProfileDeleteDialog() {
  const { isDeleteOpen, deletingId, deletingName, closeDelete } = useProfileModalStore();
    ...
    <Dialog open={isDeleteOpen} onOpenChange={(open) => !open && closeDelete()}>
      <DialogContent className="max-w-md">
        <DialogHeader className="space-y-2">
          ...
          <DialogTitle className="text-lg font-bold">Delete Profile Info</DialogTitle>
          <DialogDescription>
