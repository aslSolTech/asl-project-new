...
import { useDeleteApiKeysMutation } from "../hooks";
import { AlertTriangle, Trash2 } from "lucide-react";

export function ApiKeysDeleteDialog() {
  const { isDeleteOpen, deletingId, deletingName, closeDelete } = useApiKeysModalStore();
  const deleteMutation = useDeleteApiKeysMutation();
    ...
    <Dialog open={isDeleteOpen} onOpenChange={(open) => !open && closeDelete()}>
      <DialogContent className="max-w-md">
        <DialogHeader className="space-y-2">
          ...
          <DialogTitle className="text-lg font-bold">Delete API Keys</DialogTitle>
          <DialogDescription>
