...
import { notificationFieldsConfig } from "../constants";
import { useCreateNotificationMutation, useUpdateNotificationMutation } from "../hooks";
    ...
    <form.AppForm>
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {notificationFieldsConfig.map((field) => (
            <div key={field.key}>
              <form.AppField
                name={field.key}
                validators={{
                  onChange: ({ value }) => {
                    const shape = notificationSchema.shape[field.key as keyof typeof notificationSchema.shape];
