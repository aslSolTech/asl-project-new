...
import { useDeletePackageMutation } from "../hooks";
import { AlertTriangle, Trash2 } from "lucide-react";

export function PackageDeleteDialog() {
  const { isDeleteOpen, deletingId, deletingName, closeDelete } = usePackageModalStore();
    ...
    <Dialog open={isDeleteOpen} onOpenChange={(open) => !open && closeDelete()}>
      <DialogContent className="max-w-md">
        <DialogHeader className="space-y-2">
          ...
          <DialogTitle className="text-lg font-bold">Delete Package Wise API</DialogTitle>
          <DialogDescription>
