...
import { useDeleteUserAmountMutation } from "../hooks";
import { AlertTriangle, Trash2 } from "lucide-react";

export function UserAmountDeleteDialog() {
  const { isDeleteOpen, deletingId, deletingName, closeDelete } = useUserAmountModalStore();
  const deleteMutation = useDeleteUserAmountMutation();
    ...
    <Dialog open={isDeleteOpen} onOpenChange={(open) => !open && closeDelete()}>
      <DialogContent className="max-w-md">
        <DialogHeader className="flex flex-col items-center gap-2">
          ...
          <DialogTitle className="text-lg font-bold">Delete User Amount Wise Routing</DialogTitle>
