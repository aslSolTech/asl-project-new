...
import { useDeleteMissionMutation } from "../hooks";
import { AlertTriangle, Trash2 } from "lucide-react";

export function MissionDeleteDialog() {
  const { isDeleteOpen, deletingId, deletingName, closeDelete } = useMissionModalStore();
    ...
    <Dialog open={isDeleteOpen} onOpenChange={(open) => !open && closeDelete()}>
      <DialogContent className="max-w-md">
        <DialogHeader className="flex flex-col items-center gap-2">
          ...
          <DialogTitle className="text-lg font-bold">Delete Mission</DialogTitle>
