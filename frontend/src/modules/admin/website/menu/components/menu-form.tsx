...
import { menuSchema, MenuFormInput } from "../validations";
import { menuFieldsConfig } from "../constants";
...
export interface MenuFormProps {
  ...
  readonly initialData?: MenuRecord | null;
  readonly onSuccess?: () => void;
}

export function MenuForm({ mode, initialData, onSuccess }: MenuFormProps) {
    ...
    <form.AppForm>
      ...
      >
        <div className="grid grid-cols-1 gap-4">
          {menuFieldsConfig.map((field) => (
            <div key={field.key}>
              <form.AppField
                name={field.key}
                validators={{
                  onChange: ({ value }) => {
                    const shape = menuSchema.shape[field.key as keyof typeof menuSchema.shape];
