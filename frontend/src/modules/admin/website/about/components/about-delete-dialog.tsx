...
import { useDeleteAboutMutation } from "../hooks";
import { AlertTriangle, Trash2 } from "lucide-react";

export function AboutDeleteDialog() {
  const { isDeleteOpen, deletingId, deletingName, closeDelete } = useAboutModalStore();
    ...
    <Dialog open={isDeleteOpen} onOpenChange={(open) => !open && closeDelete()}>
      <DialogContent className="max-w-md">
        <DialogHeader className="space-y-2">
          ...
          <DialogTitle className="text-lg font-bold">Delete About Us</DialogTitle>
          <DialogDescription>
