...
import { useDeleteLoanMutation } from "../hooks";
import { AlertTriangle, Trash2 } from "lucide-react";

export function LoanDeleteDialog() {
  const { isDeleteOpen, deletingId, deletingName, closeDelete } = useLoanModalStore();
    ...
    <Dialog open={isDeleteOpen} onOpenChange={(open) => !open && closeDelete()}>
      <DialogContent className="max-w-md">
        <DialogHeader className="space-y-2">
          ...
          <DialogTitle className="text-lg font-bold">Delete Loan Report</DialogTitle>
          <DialogDescription>
