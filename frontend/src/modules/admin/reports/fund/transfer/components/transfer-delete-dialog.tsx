...
import { useDeleteTransferMutation } from "../hooks";
import { AlertTriangle, Trash2 } from "lucide-react";

export function TransferDeleteDialog() {
  const { isDeleteOpen, deletingId, deletingName, closeDelete } = useTransferModalStore();
    ...
    <Dialog open={isDeleteOpen} onOpenChange={(open) => !open && closeDelete()}>
      <DialogContent className="max-w-md">
        <DialogHeader className="space-y-2">
          ...
          <DialogTitle className="text-lg font-bold">Delete Fund Transfer Report</DialogTitle>
          <DialogDescription>
