...
import { apiTypeFieldsConfig } from "../constants";
...
export function ApiTypeForm({ mode, initialData, onSuccess }: ApiTypeFormProps) {
    ...
    <form.AppForm>
      ...
      >
        <div className="grid grid-cols-1 gap-4">
          {apiTypeFieldsConfig.map((field) => (
            <div key={field.key}>
              <form.AppField
                name={field.key}
                validators={{
                  onChange: ({ value }) => {
                    const shape = apiTypeSchema.shape[field.key as keyof typeof apiTypeSchema.shape];
