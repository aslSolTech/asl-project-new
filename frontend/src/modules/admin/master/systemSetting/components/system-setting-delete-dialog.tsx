...
import { useDeleteSystemSettingMutation } from "../hooks";
import { AlertTriangle, Trash2 } from "lucide-react";

export function SystemSettingDeleteDialog() {
  const { isDeleteOpen, deletingId, deletingName, closeDelete } = useSystemSettingModalStore();
    ...
    <Dialog open={isDeleteOpen} onOpenChange={(open) => !open && closeDelete()}>
      <DialogContent className="max-w-md">
        <DialogHeader className="space-y-2">
          ...
          <DialogTitle className="text-lg font-bold">Delete System Setting</DialogTitle>
          <DialogDescription>
