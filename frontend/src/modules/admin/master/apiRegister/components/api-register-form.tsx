...
import { apiRegisterFieldsConfig } from "../constants";
...
export function ApiRegisterForm({ mode, initialData, onSuccess }: ApiRegisterFormProps) {
  ...
  const form = useAppForm({
    ...
    onSubmit: async ({ value }) => {
      const parsed = apiRegisterSchema.safeParse(value);
    ...
    <form.AppForm>
      ...
      >
        <div className="grid grid-cols-1 gap-4">
          {apiRegisterFieldsConfig.map((field) => (
            <div key={field.key}>
              <form.AppField
                name={field.key}
                validators={{
                  onChange: ({ value }) => {
                    const shape = apiRegisterSchema.shape[field.key as keyof typeof apiRegisterSchema.shape];
