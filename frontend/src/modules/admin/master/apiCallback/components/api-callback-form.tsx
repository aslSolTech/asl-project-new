...
import { apiCallbackFieldsConfig } from "../constants";
...
export function ApiCallbackForm({ mode, initialData, onSuccess }: ApiCallbackFormProps) {
  ...
  const form = useAppForm({
    defaultValues: {
      ...
      url: initialData?.url ?? "",
      retryPolicy: initialData?.retryPolicy ?? "",
    } as ApiCallbackFormInput,
    onSubmit: async ({ value }) => {
      const parsed = apiCallbackSchema.safeParse(value);
    ...
    <form.AppForm>
      ...
      >
        <div className="grid grid-cols-1 gap-4">
          {apiCallbackFieldsConfig.map((field) => (
            <div key={field.key}>
              <form.AppField
                name={field.key}
                validators={{
                  onChange: ({ value }) => {
                    const shape = apiCallbackSchema.shape[field.key as keyof typeof apiCallbackSchema.shape];
