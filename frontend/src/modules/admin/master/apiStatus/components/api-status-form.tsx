...
import { statusForOptions } from "../constants";
import { useCreateApiStatusMutation, useUpdateApiStatusMutation } from "../hooks";
import { ApiStatusRecord, RequestParameterItem, ResponseParameterItem } from "../types";
...
export function ApiStatusForm({ mode, initialData, onSuccess }: ApiStatusFormProps) {
  ...
  // Status For options (uses API Types or predefined options)
  const computedStatusForOptions = useMemo(() => {
  ...
  const form = useAppForm({
    ...
    onSubmit: async ({ value }) => {
      ...
      const parsed = apiStatusSchema.safeParse(payload);
      ...
      >
          ...
          <div className="p-4 rounded-xl bg-muted/30 border border-border/60 space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <form.AppField
                ...
                validators={{
                  onChange: ({ value }) => {
                    const res = apiStatusSchema.shape.apiName.safeParse(value);
              ...
              >
                {(fieldState) => (
                  <FormField
                    ...
                    label="API Name"
                    type="text"
                    placeholder="e.g. Payzones Status"
                    required
              ...
              <form.AppField
                ...
                validators={{
                  onChange: ({ value }) => {
                    const res = apiStatusSchema.shape.statusFor.safeParse(value);
                    return res.success ? undefined : res.error.issues[0]?.message;
                  },
                }}
              >
                {(fieldState) => (
                  <FormField
                    ...
                    label="Status For"
                    type="select"
                    placeholder="Select Status For"
                    required
                    options={computedStatusForOptions}
                    value={fieldState.state.value ?? ""}
                    onChange={(val) =>
                      fieldState.handleChange(val as Parameters<typeof fieldState.handleChange>[0])
                    }
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="md:col-span-2">
                <form.AppField
                  ...
                  validators={{
                    onChange: ({ value }) => {
                      const res = apiStatusSchema.shape.url.safeParse(value);
                      return res.success ? undefined : res.error.issues[0]?.message;
                    },
                  }}
                >
                  {(fieldState) => (
                    <FormField
                      ...
                      label="API URL"
                      type="url"
                      placeholder="https://api.provider.com/v1/status"
                      required
                      value={fieldState.state.value ?? ""}
                      onChange={(val) =>
                        fieldState.handleChange(val as Parameters<typeof fieldState.handleChange>[0])
              ...
              <form.AppField
                ...
                validators={{
                  onChange: ({ value }) => {
                    const res = apiStatusSchema.shape.requestType.safeParse(value);
                    return res.success ? undefined : res.error.issues[0]?.message;
                  },
                }}
              >
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-[11px] text-muted-foreground">
                  Configure dynamic request parameters sent to this Status API endpoint.
                </p>
              </div>
              <Button
                type="button"
          ...
          <div className="p-4 rounded-xl bg-muted/30 border border-border/60 space-y-3">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-[11px] text-muted-foreground">
                  Map returned status response keys to system status parameters.
                </p>
              </div>
              <Button
                type="button"
          ...
          <div className="p-4 rounded-xl bg-muted/30 border border-border/60 space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <form.AppField
                ...
                validators={{
                  onChange: ({ value }) => {
                    const res = apiStatusSchema.shape.responseType.safeParse(value);
                    return res.success ? undefined : res.error.issues[0]?.message;
                  },
                }}
              >
              ...
              <form.AppField name="apiRemarks">
                {(fieldState) => (
                  <FormField
                    ...
                    label="API Remarks"
                    type="text"
                    placeholder="e.g. Transaction status check endpoint"
                    value={fieldState.state.value ?? ""}
                    onChange={(val) =>
                      fieldState.handleChange(val as Parameters<typeof fieldState.handleChange>[0])
                    }
        ...
        <div className="flex items-center justify-end gap-3 pt-4 border-t border-border bg-background">
          <form.SubscribeButton
            ...
            label={mode === "create" ? "Save Status API" : "Save Changes"}
            loadingLabel="Saving..."
            isLoading={isPending}
            disabled={isPending}
