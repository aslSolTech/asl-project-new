...
import { useDeleteCreateMutation } from "../hooks";
import { AlertTriangle, Trash2 } from "lucide-react";

export function CreateDeleteDialog() {
  const { isDeleteOpen, deletingId, deletingName, closeDelete } = useCreateModalStore();
  const deleteMutation = useDeleteCreateMutation();
    ...
    <Dialog open={isDeleteOpen} onOpenChange={(open) => !open && closeDelete()}>
      <DialogContent className="max-w-md">
        <DialogHeader className="space-y-2">
          ...
          <DialogTitle className="text-lg font-bold">Delete Create Notification</DialogTitle>
          <DialogDescription>
