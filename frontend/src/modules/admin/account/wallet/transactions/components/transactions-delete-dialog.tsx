...
import { useDeleteTransactionsMutation } from "../hooks";
import { AlertTriangle, Trash2 } from "lucide-react";

export function TransactionsDeleteDialog() {
  const { isDeleteOpen, deletingId, deletingName, closeDelete } = useTransactionsModalStore();
    ...
    <Dialog open={isDeleteOpen} onOpenChange={(open) => !open && closeDelete()}>
      <DialogContent className="max-w-md">
        <DialogHeader className="space-y-2">
          ...
          <DialogTitle className="text-lg font-bold">Delete Wallet Transactions</DialogTitle>
          <DialogDescription>
