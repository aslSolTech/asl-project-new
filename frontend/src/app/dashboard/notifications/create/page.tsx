...
import { useCreateListQuery } from "@/modules/admin/notifications/create/hooks";
import { useCreateModalStore } from "@/modules/admin/notifications/create/stores/useCreateModalStore";
import { CreateRecord } from "@/modules/admin/notifications/create/types";
import { CreateModal } from "@/modules/admin/notifications/create/components/create-modal";
import { CreateDeleteDialog } from "@/modules/admin/notifications/create/components/create-delete-dialog";
import {
  Bell,
  Plus,
  Edit2,
...
function TitleHeader({ column }: Readonly<{ column: Column<AppTableFeatures, CreateRecord, unknown> }>) {
  return <DataTableColumnHeader column={column} title="Notification Title" />;
}

function TitleCell({ row }: Readonly<{ row: Row<AppTableFeatures, CreateRecord> }>) {
...
function MessageHeader({ column }: Readonly<{ column: Column<AppTableFeatures, CreateRecord, unknown> }>) {
  return <DataTableColumnHeader column={column} title="Message" />;
}

function MessageCell({ row }: Readonly<{ row: Row<AppTableFeatures, CreateRecord> }>) {
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs sm:text-sm text-muted-foreground">
              Manage all official create notification configurations.
            </p>
          </div>
        </div>
