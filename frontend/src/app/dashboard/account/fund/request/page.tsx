...
import { RequestModal } from "@/modules/admin/account/fund/request/components/request-modal";
...
function IdHeader({ column }: Readonly<{ column: Column<AppTableFeatures, RequestRecord, unknown> }>) {
  return <DataTableColumnHeader column={column} title="ID" />;
}

function IdCell({ row }: Readonly<{ row: Row<AppTableFeatures, RequestRecord> }>) {
  return (
...
function BankNameHeader({ column }: Readonly<{ column: Column<AppTableFeatures, RequestRecord, unknown> }>) {
  return <DataTableColumnHeader column={column} title="Bank" />;
}

function BankNameCell({ row }: Readonly<{ row: Row<AppTableFeatures, RequestRecord> }>) {
...
function AmountHeader({ column }: Readonly<{ column: Column<AppTableFeatures, RequestRecord, unknown> }>) {
  return <DataTableColumnHeader column={column} title="Amount" />;
}

function AmountCell({ row }: Readonly<{ row: Row<AppTableFeatures, RequestRecord> }>) {
...
function ActionsCell({ row }: Readonly<{ row: Row<AppTableFeatures, RequestRecord> }>) {
  const { openEdit, openDelete } = useRequestModalStore();
...
export default function RequestPage() {
  ...
  const { openCreate } = useRequestModalStore();
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs sm:text-sm text-muted-foreground">
              Manage all official fund request configurations.
            </p>
          </div>
        </div>
