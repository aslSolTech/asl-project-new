...
import { useBalanceModalStore } from "@/modules/admin/account/wallet/balance/stores/useBalanceModalStore";
import { BalanceRecord, WalletTypeRecord } from "@/modules/admin/account/wallet/balance/types";
import { BalanceModal } from "@/modules/admin/account/wallet/balance/components/balance-modal";
import { BalanceDeleteDialog } from "@/modules/admin/account/wallet/balance/components/balance-delete-dialog";
...
function WalletTypeHeader({ column }:Readonly<{ column: Column<AppTableFeatures, BalanceRecord, unknown> }>) {
...
function WalletTypeCell({ row, walletTypes }: Readonly<{ row: Row<AppTableFeatures, BalanceRecord>; walletTypes: WalletTypeRecord[] }>) {
...
function TrxnDescriptionHeader({ column }: Readonly<{ column: Column<AppTableFeatures, BalanceRecord, unknown> }>) {
  return <DataTableColumnHeader column={column} title="Transaction Description" />;
}

// Columns helper components
function TrxnDescriptionCell({ row }: Readonly<{ row: Row<AppTableFeatures, BalanceRecord> }>) {
  const val = row.original.trxnDescription;
  return <span className="text-sm text-muted-foreground">{val || "-"}</span>;
}

function TrxnDateHeader({ column }: Readonly<{ column: Column<AppTableFeatures, BalanceRecord, unknown> }>) {
  return <DataTableColumnHeader column={column} title="Transaction Date" />;
}

function TrxnDateCell({ row }: Readonly<{ row: Row<AppTableFeatures, BalanceRecord> }>) {
...
export default function BalancePage() {
  ...
  const { openCreate: openBalanceCreate } = useBalanceModalStore();
  ...
  const balanceColumns = useMemo<ColumnDef<AppTableFeatures, BalanceRecord, unknown>[]>(
    () => [
      {
        accessorKey: "id",
        header: IdHeader,
