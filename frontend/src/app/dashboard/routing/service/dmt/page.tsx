...
import { useDmtListQuery } from "@/modules/admin/routing/service/dmt/hooks";
import { useDmtModalStore } from "@/modules/admin/routing/service/dmt/stores/useDmtModalStore";
import { DmtRecord } from "@/modules/admin/routing/service/dmt/types";
import { DmtModal } from "@/modules/admin/routing/service/dmt/components/dmt-modal";
import { DmtDeleteDialog } from "@/modules/admin/routing/service/dmt/components/dmt-delete-dialog";
import {
  Route,
  Plus,
  Edit2,
...
function BankHeader({ column }: Readonly<{ column: Column<AppTableFeatures, DmtRecord, unknown> }>) {
  return <DataTableColumnHeader column={column} title="Bank" />;
}

function BankCell({ row }: Readonly<{ row: Row<AppTableFeatures, DmtRecord> }>) {
  const val = row.original.bank;
...
function ApiHeader({ column }: Readonly<{ column: Column<AppTableFeatures, DmtRecord, unknown> }>) {
  return <DataTableColumnHeader column={column} title="API Partner" />;
}

function ApiCell({ row }: Readonly<{ row: Row<AppTableFeatures, DmtRecord> }>) {
  ...
  return (
    <div className="flex items-center gap-1.5">
      ...
      <Button
        variant="ghost"
        size="icon-sm"
        onClick={() => openDelete(record.id, record.bank || record.id)}
        title="Delete"
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs sm:text-sm text-muted-foreground">
              Manage all official dmt api configurations.
            </p>
          </div>
        </div>
