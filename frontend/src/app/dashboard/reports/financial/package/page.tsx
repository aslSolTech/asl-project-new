...
import { usePackageListQuery } from "@/modules/admin/reports/financial/package/hooks";
import { usePackageModalStore } from "@/modules/admin/reports/financial/package/stores/usePackageModalStore";
import { PackageRecord } from "@/modules/admin/reports/financial/package/types";
import { PackageModal } from "@/modules/admin/reports/financial/package/components/package-modal";
import { PackageDeleteDialog } from "@/modules/admin/reports/financial/package/components/package-delete-dialog";
import {
  FileText,
  Plus,
  Edit2,
...
function TxIdHeader({ column }: Readonly<{ column: Column<AppTableFeatures, PackageRecord, unknown> }>) {
  return <DataTableColumnHeader column={column} title="Transaction ID" />;
}

function TxIdCell({ row }: Readonly<{ row: Row<AppTableFeatures, PackageRecord> }>) {
...
function AmountHeader({ column }: Readonly<{ column: Column<AppTableFeatures, PackageRecord, unknown> }>) {
  return <DataTableColumnHeader column={column} title="Amount" />;
}

function AmountCell({ row }: Readonly<{ row: Row<AppTableFeatures, PackageRecord> }>) {
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-foreground flex items-center gap-2">
              Package Report Setup
            </h1>
            <p className="text-xs sm:text-sm text-muted-foreground">
              Manage all official package report configurations.
            </p>
          </div>
        </div>
