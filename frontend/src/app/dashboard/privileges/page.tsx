...
function IdHeader({ column }: Readonly<{ column: Column<AppTableFeatures, PrivilegeRecord, unknown> }>) {
  return <DataTableColumnHeader column={column} title="ID" />;
}

function IdCell({ row }: Readonly<{ row: Row<AppTableFeatures, PrivilegeRecord> }>) {
  return (
...
function RoleHeader({ column }: Readonly<{ column: Column<AppTableFeatures, PrivilegeRecord, unknown> }>) {
  return <DataTableColumnHeader column={column} title="Role" />;
}

function RoleCell({ row }: Readonly<{ row: Row<AppTableFeatures, PrivilegeRecord> }>) {
...
function AccessHeader({ column }: Readonly<{ column: Column<AppTableFeatures, PrivilegeRecord, unknown> }>) {
  return <DataTableColumnHeader column={column} title="Access Level" />;
}

function AccessCell({ row }: Readonly<{ row: Row<AppTableFeatures, PrivilegeRecord> }>) {
...
function StatusHeader({ column }: Readonly<{ column: Column<AppTableFeatures, PrivilegeRecord, unknown> }>) {
  return <DataTableColumnHeader column={column} title="Status" />;
}

function StatusCell({ row }: Readonly<{ row: Row<AppTableFeatures, PrivilegeRecord> }>) {
...
function ActionsCell({ row }: Readonly<{ row: Row<AppTableFeatures, PrivilegeRecord> }>) {
  const { openEdit, openDelete } = usePrivilegeModalStore();
...
export default function SettingsPage() {
  const { data: listData, isLoading, isError, refetch } = usePrivilegeListQuery();
  const { openCreate } = usePrivilegeModalStore();

  const displayData = useMemo<PrivilegeRecord[]>(() => {
  ...
  const columns = useMemo<ColumnDef<AppTableFeatures, PrivilegeRecord, unknown>[]>(
    () => [
      {
        accessorKey: "id",
        header: IdHeader,
