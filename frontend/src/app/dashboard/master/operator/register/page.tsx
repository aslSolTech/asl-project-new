...
function OperatorNameCell({ row }: Readonly<{ row: Row<AppTableFeatures, OperatorRegisterRecord> }>) {
  const val = row.original.operatorName;
...
function CategoryHeader({ column }: Readonly<{ column: Column<AppTableFeatures, OperatorRegisterRecord, unknown> }>) {
  return <DataTableColumnHeader column={column} title="Category Code" />;
}

function CategoryCell({ row }: Readonly<{ row: Row<AppTableFeatures, OperatorRegisterRecord> }>) {
...
function CodeHeader({ column }: Readonly<{ column: Column<AppTableFeatures, OperatorRegisterRecord, unknown> }>) {
  return <DataTableColumnHeader column={column} title="Operator Shortcode" />;
}

function CodeCell({ row }: Readonly<{ row: Row<AppTableFeatures, OperatorRegisterRecord> }>) {
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-foreground flex items-center gap-2">
              Operator Register Setup
            </h1>
            <p className="text-xs sm:text-sm text-muted-foreground">
              Manage all official operator register configurations.
            </p>
          </div>
        </div>
