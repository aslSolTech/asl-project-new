...
import { usePayoutListQuery } from "@/modules/admin/reports/financial/payout/hooks";
import { usePayoutModalStore } from "@/modules/admin/reports/financial/payout/stores/usePayoutModalStore";
import { PayoutRecord } from "@/modules/admin/reports/financial/payout/types";
import { PayoutModal } from "@/modules/admin/reports/financial/payout/components/payout-modal";
import { PayoutDeleteDialog } from "@/modules/admin/reports/financial/payout/components/payout-delete-dialog";
import {
  FileText,
  Plus,
  Edit2,
...
function TxIdHeader({ column }: Readonly<{ column: Column<AppTableFeatures, PayoutRecord, unknown> }>) {
  return <DataTableColumnHeader column={column} title="Transaction ID" />;
}

function TxIdCell({ row }: Readonly<{ row: Row<AppTableFeatures, PayoutRecord> }>) {
...
function AmountHeader({ column }: Readonly<{ column: Column<AppTableFeatures, PayoutRecord, unknown> }>) {
  return <DataTableColumnHeader column={column} title="Amount" />;
}

function AmountCell({ row }: Readonly<{ row: Row<AppTableFeatures, PayoutRecord> }>) {
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs sm:text-sm text-muted-foreground">
              Manage all official payout report configurations.
            </p>
          </div>
        </div>
