...
import { usePackageModalStore } from "@/modules/admin/packages/stores/usePackageModalStore";
import { PackageRecord } from "@/modules/admin/packages/types";
import { PackageModal } from "@/modules/admin/packages/components/package-modal";
import { PackageDeleteDialog } from "@/modules/admin/packages/components/package-delete-dialog";
import {
  Package,
  Plus,
  Edit2,
...
function PackageNameHeader({ column }: Readonly<{ column: Column<AppTableFeatures, PackageRecord, unknown> }>) {
  return <DataTableColumnHeader column={column} title="Package Name" />;
}

function PackageNameCell({ row }: Readonly<{ row: Row<AppTableFeatures, PackageRecord> }>) {
  return <span className="text-sm font-medium text-foreground">{row.original.packageName || "-"}</span>;
}

function TrialPeriodHeader({ column }: Readonly<{ column: Column<AppTableFeatures, PackageRecord, unknown> }>) {
  return <DataTableColumnHeader column={column} title="Trial Periods (Days)" />;
}

function TrialPeriodCell({ row }: Readonly<{ row: Row<AppTableFeatures, PackageRecord> }>) {
  return <span className="text-sm text-foreground">{row.original.trialPeriod ?? 0}</span>;
}

function PackageChargeHeader({ column }: Readonly<{ column: Column<AppTableFeatures, PackageRecord, unknown> }>) {
  return <DataTableColumnHeader column={column} title="Package Charge" />;
}

function PackageChargeCell({ row }: Readonly<{ row: Row<AppTableFeatures, PackageRecord> }>) {
  return <span className="text-sm font-semibold text-foreground">₹{row.original.packageCharge ?? 0}</span>;
}

function IsDefaultHeader({ column }: Readonly<{ column: Column<AppTableFeatures, PackageRecord, unknown> }>) {
  return <DataTableColumnHeader column={column} title="Is Default" />;
}

function IsDefaultCell({ row }: Readonly<{ row: Row<AppTableFeatures, PackageRecord> }>) {
  const val = String(row.original.isDefault || "").toUpperCase();
  if (val === "Y" || val === "YES") {
    return <Badge variant="default" className="text-xs uppercase bg-emerald-500/10 text-emerald-500 hover:bg-emerald-500/20 border-emerald-500/20">YES</Badge>;
  }
  return <Badge variant="outline" className="text-xs uppercase">NO</Badge>;
}
