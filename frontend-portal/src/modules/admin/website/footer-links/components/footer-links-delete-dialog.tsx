...
import { useDeleteFooterLinksMutation } from "../hooks";
import { AlertTriangle, Trash2 } from "lucide-react";

export function FooterLinksDeleteDialog() {
  const { isDeleteOpen, deletingId, deletingName, closeDelete } = useFooterLinksModalStore();
  const deleteMutation = useDeleteFooterLinksMutation();
    ...
    <Dialog open={isDeleteOpen} onOpenChange={(open) => !open && closeDelete()}>
      <DialogContent className="max-w-md">
        <DialogHeader className="flex flex-col items-center gap-2">
          ...
          <DialogTitle className="text-lg font-bold">Delete Footer Links</DialogTitle>
