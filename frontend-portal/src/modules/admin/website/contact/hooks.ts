...
export function useCreateContactMutation() {
  ...
  return useApiMutation<ContactRecord, Error, CreateContactPayload>(
    CONTACT_API_ENDPOINTS.CREATE,
    {
      ...
      options: {
        onSuccess: () => {
          toast.success("Contact Us Config created successfully!");
...
export function useUpdateContactMutation() {
  ...
  return useApiMutation<ContactRecord, Error, UpdateContactPayload>(
    (variables) => CONTACT_API_ENDPOINTS.UPDATE(variables.id),
    {
      ...
      options: {
        onSuccess: () => {
          toast.success("Contact Us Config updated successfully!");
          void queryClient.invalidateQueries({ queryKey: contactKeys.all });
        },
        onError: (error: Error) => {
...
export function useDeleteContactMutation() {
  ...
  return useApiMutation<{ success: boolean; message?: string }, Error, string>(
    (id) => CONTACT_API_ENDPOINTS.DELETE(id),
    {
      ...
      options: {
        onSuccess: () => {
          toast.success("Contact Us Config deleted successfully!");
          void queryClient.invalidateQueries({ queryKey: contactKeys.all });
        },
        onError: (error: Error) => {
