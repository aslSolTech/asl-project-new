...
import { useDeleteAmountMutation } from "../hooks";
import { AlertTriangle, Trash2 } from "lucide-react";

export function AmountDeleteDialog() {
  const { isDeleteOpen, deletingId, deletingName, closeDelete } = useAmountModalStore();
  const deleteMutation = useDeleteAmountMutation();
    ...
    <Dialog open={isDeleteOpen} onOpenChange={(open) => !open && closeDelete()}>
      <DialogContent className="max-w-md">
        <DialogHeader className="flex flex-col items-center gap-2">
          ...
          <DialogTitle className="text-lg font-bold">Delete Amount Route</DialogTitle>
          <DialogDescription className="text-center">
            Are you sure you want to delete{" "}
            <span className="font-semibold text-foreground">
              {deletingName ?? "this route"}
