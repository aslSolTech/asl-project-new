...
import { useDeleteAepsDashboardMutation } from "../hooks";
import { AlertTriangle, Trash2 } from "lucide-react";

export function AepsDashboardDeleteDialog() {
  const { isDeleteOpen, deletingId, deletingName, closeDelete } = useAepsDashboardModalStore();
  const deleteMutation = useDeleteAepsDashboardMutation();
    ...
    <Dialog open={isDeleteOpen} onOpenChange={(open) => !open && closeDelete()}>
      <DialogContent className="max-w-md">
        <DialogHeader className="flex flex-col items-center gap-2">
          ...
          <DialogTitle className="text-lg font-bold">Delete AEPS Dashboard</DialogTitle>
