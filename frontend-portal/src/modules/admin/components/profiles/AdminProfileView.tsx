...
import { AVATAR_OPTIONS } from "@/modules/admin/constants";
import {
  adminProfileSchema,
  adminChangePasswordSchema,
  AdminProfileFormInput,
  AdminChangePasswordFormInput,
} from "@/modules/admin/validations";

interface HeaderBannerProps {
  readonly profile: ReturnType<typeof useAdminProfileStore.getState>["profile"];
}
...
interface ProfileFormCardProps {
  readonly formData: AdminProfileFormInput;
  readonly profileErrors: Record<string, string>;
  readonly isPending: boolean;
  readonly onChangeField: (field: keyof AdminProfileFormInput, val: string) => void;
...
interface SecurityTabProps {
  readonly passwords: AdminChangePasswordFormInput;
  readonly passwordErrors: Record<string, string>;
  readonly showPassword: boolean;
  readonly isPending: boolean;
  readonly onChangePassword: (field: keyof AdminChangePasswordFormInput, val: string) => void;
...
interface LoginHistorySidebarProps {
  readonly loginHistory?: ReturnType<typeof useAdminProfileStore.getState>["profile"]["loginHistory"];
}
...
export const AdminProfileView = memo(function AdminProfileView() {
  ...
  const { profile, updateProfile, updateAvatar, resetProfile } = useAdminProfileStore();
  const [isPending, startTransition] = useTransition();

  // General Profile Form state
  const [formData, setFormData] = useState<AdminProfileFormInput>({
    name: profile.name,
    email: profile.email,
  ...
  // Password state
  const [passwords, setPasswords] = useState<AdminChangePasswordFormInput>({
    currentPassword: "",
    newPassword: "",
    confirmPassword: "",
  });
  ...
  const handleProfileFieldChange = (field: keyof AdminProfileFormInput, val: string) => {
  ...
  const handlePasswordFieldChange = (field: keyof AdminChangePasswordFormInput, val: string) => {
  ...
  const handlePasswordChange = (e: React.SyntheticEvent) => {
    e.preventDefault();
    const result = adminChangePasswordSchema.safeParse(passwords);
