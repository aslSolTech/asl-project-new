...
function ChartTooltipContent({
  ...
  return (
    <div
      className={cn(
        "grid min-w-[8rem] items-start gap-1.5 rounded-xl border border-border/80 bg-background/95 p-2.5 text-xs shadow-xl backdrop-blur-md",
        className
      )}
    >
...
function getPayloadConfigFromPayload(
  ...
  if (
    key in config &&
    typeof config[key as keyof typeof config] === "object"
  ) {
    configLabelKey = key;
  } else if (
    payloadPayload &&
