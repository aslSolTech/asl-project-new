...
import { useDeleteDailyPayoutMutation } from "../hooks";
import { AlertTriangle, Trash2 } from "lucide-react";

export function DailyPayoutDeleteDialog() {
  const { isDeleteOpen, deletingId, deletingName, closeDelete } = useDailyPayoutModalStore();
  const deleteMutation = useDeleteDailyPayoutMutation();
    ...
    <Dialog open={isDeleteOpen} onOpenChange={(open) => !open && closeDelete()}>
      <DialogContent className="max-w-md">
        <DialogHeader className="flex flex-col items-center gap-2">
          ...
          <DialogTitle className="text-lg font-bold">Delete Daily Payout</DialogTitle>
