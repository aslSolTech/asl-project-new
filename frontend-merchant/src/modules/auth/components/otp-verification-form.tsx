    ...
    >
      ...
      >
        ...
        <motion.div variants={FORM_ITEM_VARIANTS} className="flex items-center justify-between text-xs text-muted-foreground pt-1">
          ...
          {timer > 0 ? (
            <span className="font-mono text-primary">Resend in {timer}s</span>
          ) : (
            <Button
              variant={"link"}
              onClick={handleResend}
              disabled={isLoading}
              className="text-primary font-semibold hover:no-underline inline-flex items-center space-x-1 p-0 h-auto"
            >
