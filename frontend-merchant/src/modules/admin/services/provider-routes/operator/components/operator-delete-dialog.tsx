...
import { useDeleteOperatorMutation } from "../hooks";
import { AlertTriangle, Trash2 } from "lucide-react";

export function OperatorDeleteDialog() {
  const { isDeleteOpen, deletingId, deletingName, closeDelete } = useOperatorModalStore();
    ...
    <Dialog open={isDeleteOpen} onOpenChange={(open) => !open && closeDelete()}>
      <DialogContent className="max-w-md">
        <DialogHeader className="flex flex-col items-center gap-2">
          ...
          <DialogTitle className="text-lg font-bold">Delete Operator Route</DialogTitle>
