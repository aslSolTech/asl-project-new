...
import { useDeleteBannerMutation } from "../hooks";
import { AlertTriangle, Trash2 } from "lucide-react";

export function BannerDeleteDialog() {
  const { isDeleteOpen, deletingId, deletingName, closeDelete } = useBannerModalStore();
    ...
    <Dialog open={isDeleteOpen} onOpenChange={(open) => !open && closeDelete()}>
      <DialogContent className="max-w-md">
        <DialogHeader className="flex flex-col items-center gap-2">
          ...
          <DialogTitle className="text-lg font-bold">Delete Banner</DialogTitle>
