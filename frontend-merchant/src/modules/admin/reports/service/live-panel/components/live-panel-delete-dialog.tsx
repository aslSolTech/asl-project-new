...
import { useDeleteLivePanelMutation } from "../hooks";
import { AlertTriangle, Trash2 } from "lucide-react";

export function LivePanelDeleteDialog() {
  const { isDeleteOpen, deletingId, deletingName, closeDelete } = useLivePanelModalStore();
  const deleteMutation = useDeleteLivePanelMutation();
    ...
    <Dialog open={isDeleteOpen} onOpenChange={(open) => !open && closeDelete()}>
      <DialogContent className="max-w-md">
        <DialogHeader className="flex flex-col items-center gap-2">
          ...
          <DialogTitle className="text-lg font-bold">Delete Live Panel</DialogTitle>
